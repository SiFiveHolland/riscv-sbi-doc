...
!===
NOTE: The caller is expected to iterate over all acceptable values to possibly
find one that succeeds.
The caller can choose to ignore future ISA extensions and limit the search to
known possible values.
The possible values with the Smnpm ISA extension, and without any ISA extension
that allows different values, are 0, 7, and 16.
|===
